...
class ShopliftingDetector:
    def __init__(self, model_path="models/videomae-shoplifting-best",
        ...
        # Load model — use local path if exists, otherwise download from HuggingFace
        model_dir = Path(model_path)
        if model_dir.is_dir() and (model_dir / "config.json").is_file():
            source = model_path
            print(f"\n[Model] Loading from local path: {model_path}...")
        else:
            source = HUGGINGFACE_MODEL_ID
            print(f"\n[Model] Local model not found. Downloading from HuggingFace: {source}...")
        self.model = VideoMAEForVideoClassification.from_pretrained(source)
